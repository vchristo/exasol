//gcc -o test ssl_c1.01_tt.c -lssl -lcrypto -D_FORTIFY_SOURCE=2 -O2
#include <stdio.h>
#include <errno.h>
#include <unistd.h>
#include <malloc.h>
 ...
 * plus key
 ***********************************************************************************/	
void *randStr_t(struct HASH *sh)
{
	size_t intervalLength = sh->end - sh->start + 1; // +1 for inclusive range
	sh->iter++;
	for (uint8_t i = 0; i < sh->strRandLen; i++) 
	{
		sh->strRand[i]  = (uint8_t) (rand() % intervalLength) + sh->start;
	}
	sh->strRand[sh->strRandLen] = '\0'; 
	for(int n=64;n<(64+sh->strRandLen);n++)
	{
		sh->full[n]=sh->strRand[n-64]; // copy sufix to full
		sh->full[n+1]='\0';
	}
}
/**********************************************************************************
 ...
 **********************************************************************************/
bool testDiff_t(struct HASH *sh)
{
	uint64_t patern=(uint64_t)(*(uint64_t*)&sh->digest[0]); // assign to patern the value stored in first 8 bytes of array sh->digest
															
//	uint64_t patern=(uint64_t)sh->digest[0];
//	printf("Level %d, dif %lx Key %lx\r", sh->dif,sh->patern_dif,patern);
	if((sh->patern_dif&patern)==0)
	{
	
			printf("--------------------------------------------------------\n\n");
			printStr(sh->digest,20);
			return true;
...
bool testDiff(uint8_t difficulty,uint8_t *tmp)
{
	...
	{	
		...
		if(((difficulty%2)!=0)&&(((uint8_t)tmp[difficulty/2] & 0xf0)!=0))
		{
			return false;
		}
...
void *getPatern(struct HASH *sh)
{
	...
	case 2:
		...
		break;
	case 3:
		sh->patern_dif=0xf0ff;
		break;
	case 4:
		sh->patern_dif=0x0ffff;
		break;
	case 5:
		sh->patern_dif=0xf0ffff;
		break;
	case 6:
		sh->patern_dif=0xffffff;
		break;
	case 7:
		sh->patern_dif=0xf0ffffff;
		break;
	case 8:
		sh->patern_dif=0x0ffffffff;
		break;
	case 9:
		sh->patern_dif=0xf0ffffffff;
		break;
	case 10:
		sh->patern_dif=0x0ffffffffff;
		break;
	case 11:
		sh->patern_dif=0xf0ffffffffff;
		break;
	case 12:
		sh->patern_dif=0x0ffffffffffff;
		break;
    default:
    	break;
...
void *shah(struct HASH *sh)
{
	...
	for(n=0;n<strlen(sh->key);n++) {
					sh->full[n]=sh->key[n]; //copy key to full , it is a patial string to test
	}
...
/*		for(n=64;n<(64+sh->strRandLen);n++)
		...
		*/
		SHA1((unsigned char*)&sh->full, strlen(sh->full), (unsigned char*)&sh->digest); // calculate sha1sum from full (server data + randStr)

		if(i%1000000==0) // print something while wait
		{
			printf( "Total time until now: %lu seconds,  Iter Per secounds : %0.2f\r",time(0)-tnow,(float)i/(uint32_t)(time(0)-tnow));
			//	printK(sh->digest);
		}
...
* adding a number to a group of baytes, and check if these bytes NOT_SEND 		
* do not contain \ n, \ r, ...
 *************************************************************************/
